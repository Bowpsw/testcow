...
TC18_AddEmployee
    ...
    FOR    ${i}    IN RANGE    2    ${excel.max_row+1}
        ...
        LoginPage 
            ...
            ${message}           Set Variable if    '${message_1}' == 'Not Found Alert Element'     ${Empty}       ${message_1}

            ${Error}       Set Variable if    '${Status}' == 'FAIL'      Error         No Error
            ${Suggestion}       Set Variable if    '${Error}' == 'Error' or '${Status}' == 'FAIL'       ควรแจ้งเตือนให้ผู้ใช้งานว่า "${excel.cell(${i},11).value}"     -
